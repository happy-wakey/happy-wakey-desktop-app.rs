...
Wakey client. The production Rust/Qt implementation is in `src/app_state.rs`;
the Flutter implementation is in `happy-wakey-flutter`. Both repositories keep
byte-identical Quint models and run the same deterministic traces before they
...
- eleven deterministic Quint traces passed;
...
5. passes the eleven traces in `app_state_test.qnt` as native tests;
...
services, Bluetooth stacks, filesystems, clocks, native Qt code, or hardware
never fail. Those
